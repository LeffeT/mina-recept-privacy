...
final class RecipeBackupManager: ObservableObject {
    ...
    private func deduplicateRecipesIfNeeded() {
        ...
        context.perform {
            func recipeScore(_ recipe: Recipe) -> Int {
                let ingredientCount =
                    (recipe.ingredients as? Set<IngredientEntity>)?.count ?? 0
            ...
            func merge(recipe source: Recipe, into target: Recipe) {
                ...
                let targetIngredients =
                    (target.ingredients as? Set<IngredientEntity>) ?? []
                let sourceIngredients =
                    (source.ingredients as? Set<IngredientEntity>) ?? []
            ...
            let grouped = Dictionary(grouping: recipes) { recipe in
                DemoRecipeSeeder.deduplicationKey(for: recipe)
            }

            var didChange = false

            for (_, group) in grouped where group.count > 1 {
                guard let preferred = group.max(by: {
                    recipeScore($0) < recipeScore($1)
                }) else {
                    continue
                }
...
private nonisolated struct LocalRecipeBackupSnapshot: Codable {
    let version: Int
    let recipes: [LocalRecipeBackupRecipe]
}

private nonisolated struct LocalRecipeBackupRecipe: Codable {
    let id: String
    let title: String
